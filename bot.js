...
Bot.prototype.onHelp = function () {
    ...
    if (this.djList.active && songLimit > 0) {
        this.say(helpline
            .replace(/\{theme\}/g, 'The theme is '+theTheme)
            .replace(/\{queue\}/g, 'there is a queue, type "q+" to get on it')
            .replace(/\{limit\}/g, 'there\'s a ' + songLimit + ' song limit')
            .replace(/\{afk\}/g, 'and an afk limit of '+afk+' minutes')
            );
    } else if (this.djList.active && songLimit == 0) {
        this.say(helpline
            .replace(/\{theme\}/g, 'The theme is '+theTheme)
            .replace(/\{queue\}/g, 'there is a queue, type "q+" to get on it')
            .replace(/\{limit\}/g, 'there\'s no song limit')
            .replace(/\{afk\}/g, 'and an afk limit of '+afk+' minutes')
            );
    } else if (!this.djList.active && songLimit > 0) {
        this.say(helpline
            .replace(/\{theme\}/g, 'The theme is '+theTheme)
            .replace(/\{queue\}/g, 'it\'s fastest fingers to dj')
            .replace(/\{limit\}/g, 'there\'s a ' + songLimit + ' song limit')
            .replace(/\{afk\}/g, 'and an afk limit of '+afk+' minutes')
            );
    } else if (!this.djList.active && songLimit == 0) {
        this.say(helpline
            .replace(/\{theme\}/g, 'The theme is '+theTheme)
            .replace(/\{queue\}/g, 'it\'s fastest figners to dj')
            .replace(/\{limit\}/g, 'there\'s no song limit')
            .replace(/\{afk\}/g, 'and an afk limit of '+afk+' minutes')
            );
    }
...
Bot.prototype.onQueueCommands = function() {
  this.say('queue commands: ' +
      Object.keys(this.qCommandHandlers)
	  .map(function(s) { return "*" + s; }).join(', '));
};
...
Bot.prototype.onBanCommands = function() {
  this.say('ban commands: ' +
      Object.keys(this.banCommandHandlers)
	  .map(function(s) { return "*" + s; }).join(', '));
};

Bot.prototype.onGreetCommands = function() {
  this.say('greet commands: ' +
