...
Bot.prototype.onHelp = function () {
    ...
    if (this.djList.active && songLimit > 0) {
        this.say(helpline.replace(/\{theme\}/g, theTheme).replace(/\{queue\}/g, 'there is a queue, type *addme to get on it').replace(/\{limit\}/g, 'and there\'s a ' + songLimit + ' song limit'));
    } else if (this.djList.active && songLimit == 0) {
        this.say(helpline.replace(/\{theme\}/g, theTheme).replace(/\{queue\}/g, 'there is a queue, type *addme to get on it').replace(/\{limit\}/g, 'and there is no song limit'));
    } else if (!this.djList.active && songLimit > 0) {
        this.say(helpline.replace(/\{theme\}/g, theTheme).replace(/\{queue\}/g, 'it\'s FFA to get on deck').replace(/\{limit\}/g, 'and there\'s a ' + songLimit + ' song limit'));
    } else if (!this.djList.active && songLimit == 0) {
        this.say(helpline.replace(/\{theme\}/g, theTheme).replace(/\{queue\}/g, 'it\'s FFA to get on deck').replace(/\{limit\}/g, 'and there is no song limit'));
    }
};

Bot.prototype.onQueueCommands = function () {
    this.say('Queue Commands: ' + Object.keys(this.queueCommandHandlers).map(function (s) {
        return "*" + s;
    }).join(', '));
};

Bot.prototype.onFunCommands = function () {
    this.say('Fun Commands: ' + Object.keys(this.funCommandHandlers).map(function (s) {
        return "*" + s;
    }).join(', '));
};
...
Bot.prototype.onGreetCommands = function () {
    this.say('Greeting Commands: ' + Object.keys(this.greetCommandHandlers).map(function (s) {
        return "*" + s;
    }).join(', '));
};

Bot.prototype.onAllCommands = function () {
    this.say('My Commands: ' + Object.keys(this.commandHandlers).map(function (s) {
        return "*" + s;
    }).join(', ') + ', *whorebot.');
};

Bot.prototype.onHelpModCommands = function () {
    this.say('Mod Commands: ' + Object.keys(this.modCommandHandlers).map(function (s) {
        return "*" + s;
    }).join(', '));
};

Bot.prototype.onMoreCommands = function () {
    this.say('More Commands: ' + Object.keys(this.moreCommandHandlers).map(function (s) {
        return "*" + s;
    }).join(', '));
};
...
Bot.prototype.onDrink = function (text, userid, username) {
    if (isOut != true) {
        if (bac == 0) {
            this.say(imports.rsp.sober());
            this.say('/me drinks');
            bac++;
        } else if (bac == 1) {
            this.say(imports.rsp.onedrink());
            this.say('/me drinks');
            bac++;
        } else if (bac > 1 && bac < 5) {
            this.say(imports.rsp.buzzed());
            this.say('/me drinks');
            bac++;
        } else if (bac >= 5 && bac < 10) {
            this.say(imports.rsp.drunk());
            this.say('/me drinks');
            bac++;
        } else if (bac >= 10 && bac < 15) {
            this.say(imports.rsp.wasted());
            this.say('/me drinks');
            bac++;
        } else if (bac >= 15) {
...
Bot.prototype.onLast = function (text, unused_userid, unused_username) {
    ...
    if (last) {
        ...
        this.say(this.config.messages.lastActivity.replace(/\{user\.name\}/g, subject_name).replace(/\{age\}/g, age_h + " hours"));
    } else {
...
Bot.prototype.onList = function (text, userid, username) {
    ...
    if (this.djList.length()) {
        this.say(this.config.messages.list.replace(/\{list\}/g, this.djList.list.map(this.lookupUsername.bind(this)).join(', ')));
    } else {
...
Bot.prototype.onRoomInfo = function (data) {
    ...
    if (data.success) {
        ...
        if (!this.currentSong) {
            ...
            data.room.metadata.current_song, this.users[data.room.metadata.current_dj]);
...
Bot.prototype.onNewSong = function (data) {
    ...
    if (this.afkCheck(userid, afk) == true) {
        this.say('Sorry, ' + djstats.user.name + ' , you\'ve been afk ' + afk + ' minutes, let someone else up.');
        this.ttapi.remDj(userid);
        waskicked = true;
    }
    if (songLimit > 0) {
